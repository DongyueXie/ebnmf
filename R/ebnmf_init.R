...
ebnmf_init = function(X,
  ...
  if(is.list(init)){
    L_init = init$L_init
    F_init = init$F_init
    cat('F_init should be the EF from ebnmf fit...')
    cat('\n')
...
# ebnmf_init_smooth = function(res,K,p,x,alpha,ebps_control,maxiter_init,ebpm.fn.f){
...
#' #'@export
#' ebnmf_init = function(X,K,init,init_loss,maxiter_init = 50){
#'
#'   n = nrow(X)
#'   if(is.list(init)){
#'     L_init = init$L_init
