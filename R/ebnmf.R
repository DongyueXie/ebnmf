...
ebnmf = function(X,K,
                  ...
                  maxiter_init = 100,
                  tol=1e-3,
                  ebpm.fn=c(ebpm::ebpm_gamma,ebpm::ebpm_gamma),
                  fix_F = FALSE,
                  smooth_F = TRUE,
                  smooth_control=list(),
                  warm_start=TRUE,
  ...
  for(iter in 1:maxiter){
    ...
    for(k in 1:K){
      ...
      res = ebnmf_update_rank1(Ez$rs,Ez$cs,k,ebpm.fn.l,ebpm.fn.f,res,fix_F,smooth_F,smooth_control,warm_start)
    }
...
calc_ebnmf_obj_sparse = function(x,n,p,K,res,non0_idx,alpha){
  val = sum(x$x*alpha*(res$ql$Elogl[x$i,]+res$qf$Elogf[x$j,]-log(alpha)))
  obj = val - sum(tcrossprod(res$ql$El,res$qf$Ef)) - sum(lfactorial(x$x)) + sum(res$Hl)+sum(res$Hf)
  return(obj)
}
...
ebnmf_update_rank1 = function(l_seq,f_seq,k,ebpm.fn.l,ebpm.fn.f,res,fix_F,smooth_F,ebpm_control,warm_start){
